...
public:
    ...
    Bcast(const char *shmem, const SlotClosure &progressCallback, const SlotClosure &bitmapInfo, Fetch *fetch =nullptr);

    void emitProgress();
    void emitBitmapInfo();
    void skip(int clusterNo, int length);
    
    static void bitmapInfo(const QByteArray &bitmap, QList<int> &jpegHeads, QList<int> &goodHeads, BitmapInfo &info);
